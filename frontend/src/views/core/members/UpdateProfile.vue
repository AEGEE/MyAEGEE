    ...
    <div class="tile is-child">
      <template>
        <div class="notification is-danger">
          <div class="content">
            <strong
            >Your profile has the following issues and you have to fix them before using the
              system:</strong
            >
            <ul>
      ...
      <form
        ...
        v-if="validationErrors.privacy || validationErrors.username"
      >
          ...
          <div class="control is-expanded">
            <div class="select is-fullwidth">
              <select v-model="tmpBodyId">
                <option v-for="body in bodies" v-bind:key="body.id" :value="body.id">
                  {{ body.name }}
                </option>
              </select>
            </div>
...
export default {
  ...
  methods: {
    saveUser () {
      ...
      this.axios
        .put(this.services['core'] + '/members/me', body)
        .then(() => {
          this.isSaving = false
    ...
    joinBody (bodyId, motivation) {
      this.isLoading = true
      this.axios
        .post(this.services['core'] + '/bodies/' + bodyId + '/join-requests', {
          motivation
        })
        .then(() => {
          this.$root.showSuccess(
            'Join request is sent. Please wait until a board members accepts it.'
          )
    ...
    changeEmail (newEmail) {
      ...
      this.axios
        .put(this.services['core'] + '/members/me/email', { new_email: newEmail })
        .then(() => {
          this.$root.showSuccess(
  ...
  mounted () {
    ...
    this.axios
      .get(this.services['core'] + '/members/me')
      .then((response) => {
