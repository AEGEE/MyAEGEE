...
if [ "$CONTAINER" == "all" ] || [ "$CONTAINER" == "mongoui" ]; then
    ...
    docker build -t mongoui -f ./mongoui/Dockerfile.$TAG ./mongoui || exit 10
...
if [ "$CONTAINER" == "all" ] || [ "$CONTAINER" == "omsapplications" ]; then
    ...
    docker build -t omsapplications -f ./omsapplications/Dockerfile.$TAG ./omsapplications || exit 10
...
if [ "$CONTAINER" == "all" ] || [ "$CONTAINER" == "omsapplications-frontend" ]; then
    ...
    docker build -t omsapplications-frontend -f ./omsapplications-frontend/Dockerfile.$TAG ./omsapplications-frontend || exit 10
...
if [ "$CONTAINER" == "all" ] || [ "$CONTAINER" == "stable" ] || [ "$CONTAINER" == "omscore" ]; then
    ...
    docker build -t omscore -f ../oms-core/docker/Dockerfile.$TAG ../oms-core/docker || exit 10
...
if [ "$CONTAINER" == "all" ] || [ "$CONTAINER" == "stable" ] || [ "$CONTAINER" == "omscore-nginx" ]; then
    ...
    docker build -t omscore-nginx -f ./omscore-nginx/Dockerfile.$TAG ./omscore-nginx || exit 10
...
if [ "$CONTAINER" == "all" ] || [ "$CONTAINER" == "omsserviceregistry" ]; then
    ...
    docker build -t omsserviceregistry -f ./omsserviceregistry/Dockerfile.$TAG ./omsserviceregistry || exit 10
...
if [ "$CONTAINER" == "all" ] || [ "$CONTAINER" == "stable" ] || [ "$CONTAINER" == "php-fpm" ]; then
    ...
    docker build -t php-fpm -f ./php-fpm/Dockerfile.$TAG ./php-fpm || exit 10
...
if [ "$CONTAINER" == "all" ] || [ "$CONTAINER" == "stable" ] || [ "$CONTAINER" == "traefik" ]; then
    ...
    docker build -t traefik -f ./traefik/Dockerfile.$TAG ./traefik || exit 10
