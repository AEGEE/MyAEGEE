...
describe('Events application comments', () => {
  ...
  it('should disallow changing comment if you don\'t have rights for it', (done) => {
    ...
    chai.request(server)
      .put(`/single/${event.id}/participants/${application.id}/comment`)
      .set('X-Auth-Token', 'foobar')
      .send({ board_comment: 'Not that good' })
      .end((err, res) => {
  ...
  it('should disallow changing status if the application is not found', (done) => {
    ...
    chai.request(server)
      .put(`/single/${event.id}/participants/nonexistant/comment`)
      .set('X-Auth-Token', 'foobar')
      .send({ board_comment: 'Awesome person' })
      .end((err, res) => {
    ...
    chai.request(server)
      ...
      .end((err, res) => {
        ...
        Event.findById(event.id).then((newEvent) => {
          ...
          expect(newApplication.board_comment).to.equal('Awesome person');
